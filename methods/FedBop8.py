...
class FedBop8(FederatedLearningClass):
    ...
    def __init__(self, method_name, fl_context, method_args):
        ...
        self.decay_rounds = self.get_arg(int, "decay_rounds", 200)
        ...
        # Decay-only control for the one-shot-sync experiment. The optimizer and
        # binary aggregation are intentionally identical to the sync branch, but
        # optimizer-state communication is completely disabled. Gamma still follows
        # the same decay schedule. This isolates the causal contribution of the
        # one-shot momentum synchronization at the first decay.
        self.anchor_interval = 0  # all optimizer-state anchors/syncs disabled
        ...
        self.anchor_skip_index_uplink = False  # compatibility only; no anchor round exists
    ...
    def _diag_log(self, message):
        ...
        text = f"[FedBopDecayOnlyStateMajMinProj] {message}"
    ...
    def _is_anchor_round(self, round_number):
        """Decay-only control: optimizer-state synchronization is disabled.

        Gamma still decays on exactly the same schedule as the sync experiment,
        but no client ever uploads momentum and the server never broadcasts a
        shared momentum state. Returning False here keeps every round on the
        ordinary sparse state-majority + Server-Bop + minimal-projection path.
        """
        return False
    ...
    def _aggregate_anchor_round(self, clients_models, global_model, round_number):
        ...
        if self.diag:
            ...
            self._diag_log(
                f"SERVER_FIRST_DECAY_SYNC round={round_number} mode=one_shot_first_decay_sidecar "
                f"K={k_clients} first_decay_round={self.decay_rounds} tau={self.tau:.3e} "
            ...
            if self._diag_layer_log_enabled(max(1, int(round_number))):
                for key, d in layer_diag.items():
                    self._diag_log(
                        f"SERVER_FIRST_DECAY_SYNC_LAYER round={round_number} "
                        f"mode=one_shot_first_decay_sidecar layer={key} n={d['n']} "
    ...
    def pack_client_model(self, raw_model, global_model, id):
        ...
        for key, tensor in raw_model.items():
            ...
            # The sparse model-update event stream is ALWAYS sent. On the first
            # decay round only, ternary momentum is added as a one-shot sidecar.
        ...
        if self.diag:
            ...
            self._diag_log(
                ...
                f"first_decay_sync={int(anchor_round)} projection={'shared_ternary_m_after_normal_update' if anchor_round else 'minimal_feasible'} "
            ...
            if anchor_round:
                ...
                self._diag_log(
                    f"CLIENT_FIRST_DECAY_SYNC round={round_number} id={id} decay_round={self.decay_rounds} "
                    f"skip_index_uplink={int(self.anchor_skip_index_uplink)} "
                    f"transmitted_index_votes={total_votes} "
    ...
    def aggregate(self, clients_models, global_model):
        ...
        if self._is_anchor_round(round_number) and anchor_packets != len(clients_models):
            raise RuntimeError(
                f"First-decay sync round {round_number} expected ternary momentum from all "
        ...
        # After the ordinary sparse binary aggregation has completed, prepare a
        # ONE-SHOT shared momentum state on the first gamma-decay round. This does
        # not alter global binary weights; it only prepares the optimizer-state
        # sidecar that pack_server_model broadcasts together with the decayed gamma.
        if self._is_anchor_round(round_number):
            self._aggregate_anchor_round(clients_models, global_model, round_number)
        ...
        if self.diag:
            ...
            self._diag_log(
                ...
                f"evidence=state_majority_positive_only_server_bop_minimal_feasible_projection boundary={decision_boundary:.1f} "
    ...
    def pack_server_model(self, raw_model):
        ...
        if self.diag and did_decay:
            self._diag_log(
                f"DECAY_ONLY_CONTROL round={round_number} "
                f"gamma={old_gamma:.3e}->{self.gamma:.3e} optimizer_state_sync=0"
            )
    ...
    def unpack_server_model(
        ...
        for key, global_tensor in bin_model.items():
            ...
            if anchor_sync:
                ...
                # Minimum-change feasible projection of PRIVATE momentum.
                # Bop accepts the server-selected global sign w_new iff
                #       w_new * m <= tau.
                # We solve, independently per coordinate,
                #       min_x 0.5 (x-m)^2  s.t. w_new*x <= tau.
                # The exact Euclidean projection is
                #       m* = m - [w_new*m - tau]_+ * w_new.
                # Therefore safe coordinates are untouched; only actual Bop
                # violations are clipped to the strict decision boundary.
                aligned_pre = new_global_weight * m
                violation = aligned_pre > self.tau
                excess = torch.clamp(aligned_pre - self.tau, min=0.0)
                projected_m = m - excess * new_global_weight
                ...
                if self.diag:
                    ...
                    projected_n = int(violation.sum().item())
                    violation_server_n = int((violation & server_flip).sum().item())
                    violation_rejected_n = int((violation & local_rejected).sum().item())
                    violation_other_n = projected_n - int((violation & (server_flip | local_rejected)).sum().item())
                    delta_abs_sum = float((projected_m[violation] - m[violation]).abs().sum().item()) if projected_n > 0 else 0.0

                m.copy_(projected_m)

                if self.diag:
                    aligned_post = new_global_weight * m
                    post_violation_n = int((aligned_post > self.tau).sum().item())
                    boundary_err = (aligned_post[violation] - self.tau).abs() if projected_n > 0 else None
                    err_sum = float(boundary_err.sum().item()) if boundary_err is not None else 0.0
                    err_max = float(boundary_err.max().item()) if boundary_err is not None else 0.0
                    layer_diag = {
                        "numel": n,
                        ...
                        "server_projected": violation_server_n,
                        "rejected_projected": violation_rejected_n,
                        "projected_union": projected_n,
                        "pre_trigger_projected": projected_n,
                        "post_trigger_projected": post_violation_n,
                        "post_trigger_server": int(((aligned_post > self.tau) & server_flip).sum().item()),
                        "post_trigger_rejected": int(((aligned_post > self.tau) & local_rejected).sum().item()),
                        "projection_delta_abs_sum": delta_abs_sum,
                        "server_err_abs_sum": err_sum,
                        "server_err_max": err_max,
                        "rejected_err_abs_sum": err_sum,
                        "rejected_err_max": err_max,
                        "violation_other": violation_other_n,
                    }
                    ...
                    total_server_projected += violation_server_n
                    total_rejected_projected += violation_rejected_n
                    total_projected_union += projected_n
                    total_pre_trigger_projected += projected_n
                    total_post_trigger_projected += post_violation_n
                    total_post_trigger_server += layer_diag["post_trigger_server"]
                    total_post_trigger_rejected += layer_diag["post_trigger_rejected"]
                    total_projection_delta_abs += delta_abs_sum
                    total_projection_numel += projected_n
                    total_server_projection_error_abs += err_sum
                    total_rejected_projection_error_abs += err_sum
                    max_server_projection_error = max(max_server_projection_error, err_max)
                    max_rejected_projection_error = max(max_rejected_projection_error, err_max)
        ...
        if self.diag and anchor_sync:
            ...
            self._diag_log(
                f"SYNC_FIRST_DECAY_MOMENTUM round={round_number} gamma={self.gamma:.3e} "
        ...
        if self.diag and self.has_local_update and not anchor_sync:
            ...
            self._diag_log(
                f"SYNC_MINIMAL_FEASIBLE_PROJECTION round={round_number} gamma={self.gamma:.3e} "
                f"mode=euclidean_halfspace_projection rho_unused={received_rho:.3e} "
            ...
            if self._diag_layer_log_enabled(round_number):
                for key, d in sync_layers.items():
                    ...
                    self._diag_log(
                        f"SYNC_MINIMAL_FEASIBLE_PROJECTION_LAYER round={round_number} layer={key} n={d['numel']} "
