...
def create_local_clients(train_ds_list: list, clients_list: list):
   for client_id in range(len(train_ds_list)):
      ...
      _ = Client(f"Client{client_id}", IpAddr("127.0.0.1", PORT_NUM), TrainingHyperParameters(0.001, 0.9, 1e-7), train_ds_list[client_id], model, optim.SGD, nn.CrossEntropyLoss, "cpu")
...
fedavg = FedAvg((5, weights))
model = arch.CreateModel()
server = Server(IpAddr("127.0.0.1", PORT_NUM), fedavg, test_ds, model, nn.CrossEntropyLoss, "cpu")
clients_list = []
