...
class FedPipe(FederatedLearningClass):
    ...
    def aggregate(self, clients_models, global_model):
        ...
        client_model = clients_models[0]
        round_completed = True
        for key in self.clients_pool.keys():
            if self.clients_pool[key] == False:
                round_completed = False

        global_model = client_model
        if round_completed:
            self.round_num += 1
            logger.log_normal(f"One round Completed!")
        else:
            logger.log_normal(f"Waiting for next client")
    ...
    def select_clients_to_train(self, all_clients):
        if self.clients_pool is None:
            self.clients_pool = {key:False for key in all_clients.keys()}
        
        selected_client_id, info = random.sample(list(all_clients.items()), 1)[0]
        while self.clients_pool[selected_client_id]:
            selected_client_id, info = random.sample(list(all_clients.items()), 1)[0]
        
        self.clients_pool[selected_client_id] = True
        return dict([(selected_client_id, info)])
    ...
    def calculate_dist(self, model_i, model_j):
