...
class FedBop5(FederatedLearningClass):
    ...
    def __init__(self, method_name, fl_context, method_args):
        ...
        # Periodic low-bit optimizer-state anchor. Ordinary rounds retain the best
        # sparse path found so far: temporal Bop-request majority -> positive-only
        # Server-Bop -> canonical sparse momentum projection. Every anchor_interval
        # rounds, clients additionally send a base-3 packed ternary momentum snapshot.
        # The server reconstructs their mean, takes a direct Bop decision, then sends
        # back one shared ternary momentum snapshot so every client starts the next
        # round from the same optimizer state.
        self.anchor_interval = self.get_arg(int, "anchor_interval", 5)
    ...
    def _diag_log(self, message):
        ...
        text = f"[FedBopPeriodicTernaryAnchor] {message}"
    ...
    def _aggregate_anchor_round(self, clients_models, global_model, round_number):
        """Periodic ternary optimizer-state synchronization round.

        Client uplink: Q3(m_k^end), packed as five trits/byte + one FP32 scale/layer.
        Server decision: w_g * mean_k[Q3(m_k)] > tau (direct Bop, no Server-Bop proxy).
        Downlink: Q3(mean momentum) shared by every client. Server also replaces its
        own persistent momentum with exactly this downlink reconstruction.
        """
        ...
        for key, global_tensor in global_model.items():
            ...
            for _, client_model in clients_models:
                ...
                if entry is None:
                    continue
                decoded = self._decode_ternary_entry(entry, device="cpu", dtype=torch.float32).view(-1)
                if decoded.numel() != n:
                    raise ValueError(
                        f"Anchor ternary momentum size mismatch for {key}: {decoded.numel()} != {n}"
                    )
                m_sum.add_(decoded)
                received += 1
                layer_uplink_bytes += int(torch.as_tensor(entry["packed"]).numel()) + 4
                # Count nonzero reconstructed ternary states for diagnostics only.
                layer_client_nonzero += int((decoded != 0).sum().item())

            if received == 0:
                # Fail closed: do not invent an anchor state if this layer was absent.
                missing_layers += 1
                continue

            mean_m = m_sum / float(received)
            aligned_mean = old_w * mean_m
            direct_flip = aligned_mean > self.tau

            # The downlink is itself ternary. We intentionally keep the *decision*
            # on the higher-fidelity mean of client ternaries, then quantify how much
            # a second ternarization changes the decision. The next round starts from
            # the exact shared downlink state on server and every client.
            sync_entry, sync_m, sync_diag = self._quantize_momentum_ternary(mean_m)
            sync_m = sync_m.view(-1)
            sync_flip = (old_w * sync_m) > self.tau
            decision_disagreement = int((direct_flip != sync_flip).sum().item())

            if direct_flip.any():
                mask = direct_flip.to(device=global_tensor.device, dtype=torch.bool)
                global_tensor.view(-1)[mask] *= -1
            ...
            flips = int(direct_flip.sum().item())
            total_binary += n
            total_flips += flips
        ...
        if self.diag:
            # total_client_payload_bytes is the sum over all participating clients.
            ...
            flip_rate = total_flips / max(1, total_binary)
            ...
            self._diag_log(
                f"SERVER_TERNARY_ANCHOR round={round_number} K={k_clients} interval={self.anchor_interval} "
                f"tau={self.tau:.3e} deadzone_mult={self.ternary_deadzone_mult:.3f} "
                f"global_flips={total_flips} global_flip_rate={flip_rate:.6e} "
            ...
            if self._diag_layer_log_enabled(max(1, int(round_number))):
                for key, d in layer_diag.items():
                    self._diag_log(
                        f"SERVER_TERNARY_ANCHOR_LAYER round={round_number} layer={key} n={d['n']} "
                        f"received={d['received']} flips={d['flips']} "
                        ...
                        f"mean|m_anchor|={d['mean_abs']:.3e} mean|m_sync|={d['sync_abs']:.3e} "
                        f"sync_scale={d['sync_scale']:.3e} sync_rel_rmse={d['sync_rel_rmse']:.3e}"
                    )
    ...
    def pack_client_model(self, raw_model, global_model, id):
        ...
        if self.diag:
            ...
            self._diag_log(
                f"CLIENT_BOP_REQUEST_MAJ round={round_number} id={id} gamma={self.gamma:.3e} "
    ...
    def aggregate(self, clients_models, global_model):
        ...
        if self.diag:
            ...
            self._diag_log(
                ...
                f"evidence=local_temporal_bop_request_majority_positive_only_sparse_projection_nonanchor boundary={decision_boundary:.1f} "
    ...
    def unpack_server_model(
        ...
        for key, global_tensor in bin_model.items():
            ...
            if anchor_sync:
                ...
                # f_local is the strict temporal majority of
                # q_s = 1[w_start * m_s > tau].
                ...
                if stored_vote is not None and stored_vote.numel() == client_tensor.numel():
                    ...
                    # Use the final round-start-aligned Bop request rather than
                    # endpoint parity.
                    local_vote = (old_global_weight * m) > self.tau
