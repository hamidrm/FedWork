...
class FedBop7(FederatedLearningClass):
    ...
    def __init__(self, method_name, fl_context, method_args):
        ...
        self.decay_rounds = self.get_arg(int, "decay_rounds", 100)
        ...
        self.anchor_interval = 0  # periodic anchors disabled; only first decay sync is used
        ...
        self.anchor_skip_index_uplink = False  # decay-sync round still sends normal sparse vote indices
    ...
    def _diag_log(self, message):
        ...
        text = f"[FedBopDecaySyncStateMajMinProj] {message}"
    ...
    def _is_anchor_round(self, round_number):
        """Compatibility name: True only on the FIRST gamma-decay round.

        If decay_rounds=200, clients send the one-shot ternary momentum sidecar
        in round 200. The ordinary sparse binary vote packet is still sent and
        aggregated in that same round. No periodic optimizer-state anchors exist.
        """
        try:
            r = int(round_number)
        except Exception:
            return False
        return self.decay_rounds > 0 and r == int(self.decay_rounds)
